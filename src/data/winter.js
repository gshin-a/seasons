export const introductionList = [
  {
    id: 1,
    title: "Introduction to Winter in Korea",
    description: `Winter in Korea is an enchanting season that captivates visitors withits breathtaking snow-covered landscapes, unique cultural experiences,and delicious seasonal cuisine. In this article, we will explore several subheadings that will give foreigners an understanding of what to expect during winter in Korea.`,
  },
  {
    id: 2,
    title: "Climate and Temperatures",
    description: `Winter in Korea can be chilly, with temperatures dropping to around -10to 5 degrees Celsius (14 to 41 degrees Fahrenheit). The season typically lasts from December to February, with January being the coldest month.
    
    Winter in Korea can be chilly, with temperatures dropping to around -10to 5 degrees Celsius (14 to 41 degrees Fahrenheit). The season typically lasts from December to February, with January being the coldest month.
    
    It's important to pack warmly when visiting Korea during the winter season, with hats, gloves, scarves, and thick coats being essential items to keep warm.`,
  },
  {
    id: 3,
    title: "Winter Activities",
    description: `Winter in Korea offers a unique set of activities and experiences that are not available during other seasons. Skiing and snowboarding are some of the most popular winter sports, with the country's many ski resorts offering slopes for all skill levels.
    
    Another popular activity during winter is ice fishing, where visitors can catch fish through a hole in the ice. Ice skating and sledding are also popular activities for families to enjoy together.
    
    Visitors can also experience traditional Korean culture during thewinter season. Many festivals and events take place, including thePyeongchang Trout Festival, the Hwacheon Sancheoneo Ice Festival, and the Jeju Fire Festival.`,
  },
  {
    id: 4,
    title: "Winter Festivals and Events",
    description: `Winter in Korea is also a time of celebration, with a variety of festivals and events that showcase Korean culture and traditions. These events offer visitors a chance to experience unique winter activities and participate in traditional Korean customs.
    
    One of the most famous winter festivals in Korea is the Hwacheon Sancheoneo Ice Festival, held annually in Hwacheon county. This festival celebrates ice fishing, and visitors can participate in catching mountain trout from a frozen river, as well as enjoy various winter activities like sledding and ice-skating.
    
    Another popular winter festival is the Pyeongchang Trout Festival, held in Gangwon Province. This festival is also focused on ice fishing, and visitors can enjoy fresh grilled trout, traditional performances, and snowman-making competitions.
    
    Other winter festivals and events in Korea include the Seoul Lantern Festival, where visitors can admire hundreds of illuminated lanterns, and the Taebaeksan Mountain Snow Festival, which offers a range of winter activities and beautiful snow sculptures.`,
  },
  {
    id: 5,
    title: "Winter Food and Cuisine",
    description: `Korean winter cuisine is all about hearty and warming dishes thatprovide comfort during the cold weather. One of the most popular Korean winter dishes is hot pot, or jeongol, which is a communal stew made with a variety of ingredients, such as beef, seafood, and vegetables. Another winter favorite is dak-galbi, a spicy stir-fried chicken dish that is served with rice cakes and vegetables.
    
    Korean winter cuisine also includes a variety of soups and stews, such as kimchi jjigae, a spicy kimchi stew, and doenjang jjigae, a soybean paste stew that is served with tofu and vegetables. Traditional Korean snacks like hotteok, a sweet pancake filled with cinnamon and brownsugar, and bungeoppang, a fish-shaped pastry filled with sweet red bean paste, are also popular during the winter season.`,
  },
  {
    id: 6,
    title: "Conclusion",
    description: `Overall, Korea's winter season offers a unique and unforgettable experience, with a variety of winter sports, festivals, and events that showcase the country's culture and traditions. Despite the coldtemperatures, visitors can enjoy a range of outdoor activities and indulge in delicious winter cuisine. If you're planning a visit to Korea during the winter season, be sure to participate in some of thefestivals and activities and taste!`,
  },
];

export const festivalActivityList = [
  "Pocheon Dongjanggun Festival",
  "E-World Starlight Festival",
  "Pyeongchang Trout Festival",
  "Hwacheon Sancheoneo Ice Festival",
  "Chilgapsan Ice Fountain Festival",
  "Five Colored Star Garden Exhibition",
  "Taebaek Mountain Snow Festival",
  "Boseong Tea Field Light Festival",
  "Busan Christmas Festival",
  "Byeokchoji Arboretum Light Festival",
];

export const foodActivityList = [
  "Bean curd Stew",
  "Dried soybean paste soup",
  "Kimchi hot pot",
  "Fish Cake Soup",
  "Red bean porridge",
  "Fish-shaped bun",
  "Hotteok",
  "Steamed bun",
  "Citron tea",
  "Rice-cake soup",
];

export const tipList = [
  {
    imgNum: 1,
    title: "Dress warmly",
    description:
      "Winters in Korea can be very cold, with temperatures often dipping below freezing. Dress in layers and wear warm, boots, a heavy coat, gloves, and a hat.",
    link: "https://english.visitkorea.or.kr/enu/ATR/SI_ENG_2_11_1.jsp",
  },
  {
    imgNum: 2,
    title: "Try Korean street food",
    description:
      "Korean street food is delicious all year round, but it's especially satisfying in the winter months. Warm up with some hotteok (sweet pancakes), tteokbokki (spicy rice cakes), or mandu (dumplings).",
    link: "https://10mag.com/10-korean-winter-street-foods-to-beat-the-cold/",
  },
  {
    imgNum: 3,
    title: "Visit a hot spring",
    description:
      "There's nothing more relaxing than soaking in a hot spring on a cold winter day.",
    link: "https://www.koreatimes.co.kr/www/nation/2023/01/631_172128.html",
  },
  {
    imgNum: 4,
    title: "Go ice skating",
    description:
      "Ice skating is a popular winter activity in Korea, with many outdoor rinks set up in public spaces. Put on your skates and glide across the ice, enjoying the winter scenery.",
    link: "https://www.trazy.com/theme/ice_skating_seoul",
  },
  {
    imgNum: 5,
    title: "Visit a winter festival",
    description:
      "Korea has many winter festivals, such as the Hwacheon Sancheoneo Ice Festival and the Taebaeksan Mountain Snow Festival.",
    link: "https://english.visitkorea.or.kr/enu/ATR/SI_ENG_2_11_4.jsp",
  },
  {
    imgNum: 6,
    title: "Visit a ski resort",
    description:
      "Korea has many excellent ski resorts, such as Yongpyong Ski Resort and Alpensia Ski Resort. Hit the slopes and enjoy the thrill of skiing or snowboarding.",
    link: "https://www.klook.com/en-SG/blog/best-ski-resorts-in-south-korea/",
  },
  {
    imgNum: 7,
    title: "Try Korean hot pot",
    description:
      "Korean hot pot, or jjigae, is a hearty and comforting dish that's perfect for the winter months. Warm up with a bowl of hot, spicy stew.",
    link: "https://www.mangoplate.com/en/top_lists/eng_copy_1422_jeongol",
  },
  {
    imgNum: 8,
    title: "Visit indoor attractions",
    description:
      "Consider visiting indoor attractions like museums, art galleries, or indoor theme parks.",
    link: "https://www.trazy.com/event/seoul-winter-indoor-activities",
  },
];

export const JejuGalleryList = [
  {
    id: 1,
    imgSrc: process.env.PUBLIC_URL + `/assets/gallery-content-jeju-1.jpg`,
    description: "Dolhareubang in Jeju Island",
  },
  {
    id: 2,
    imgSrc: process.env.PUBLIC_URL + `/assets/gallery-content-jeju-2.jpg`,
    description: "Jeju tourist attractions that you can't miss this spring",
  },
  {
    id: 3,
    imgSrc: process.env.PUBLIC_URL + `/assets/gallery-content-jeju-3.jpg`,
    description:
      "Jeju, where spring has come first, is in full bloom with cherry blossoms and rape flowers",
  },
  {
    id: 4,
    imgSrc: process.env.PUBLIC_URL + `/assets/gallery-content-jeju-4.jpg`,
    description: "7 reasons why you should travel to Jeju in April",
  },
  {
    id: 5,
    imgSrc: process.env.PUBLIC_URL + `/assets/gallery-content-jeju-5.jpg`,
    description: "Jeju Spring Full of Flower Scent",
  },
  {
    id: 6,
    imgSrc: process.env.PUBLIC_URL + `/assets/gallery-content-jeju-6.jpg`,
    description:
      "Recommended trekking course to feel the spring of Hallasan Mountain",
  },
];

export const SeoulGalleryList = [
  {
    id: 1,
    imgSrc: process.env.PUBLIC_URL + `/assets/gallery-content-seoul-1.jpg`,
    description: "a two-legged spring outing in Seoul",
  },
  {
    id: 2,
    imgSrc: process.env.PUBLIC_URL + `/assets/gallery-content-seoul-2.jpg`,
    description: "Seoul Spring Date. Best 5 for couples",
  },
  {
    id: 3,
    imgSrc: process.env.PUBLIC_URL + `/assets/gallery-content-seoul-3.jpg`,
    description: "Places to go for a walk in Seoul's spring",
  },
  {
    id: 4,
    imgSrc: process.env.PUBLIC_URL + `/assets/gallery-content-seoul-4.jpg`,
    description:
      "This is the spring picnic in Seoul, the spring scenery of Seoul Forest",
  },
  {
    id: 5,
    imgSrc: process.env.PUBLIC_URL + `/assets/gallery-content-seoul-5.jpg`,
    description: "Enjoy Spring Flowers at Seoul Land!",
  },
  {
    id: 6,
    imgSrc: process.env.PUBLIC_URL + `/assets/gallery-content-seoul-6.jpg`,
    description: "Namsan Tower, a good place to go on a spring trip in Seoul",
  },
];

export const BusanGalleryList = [
  {
    id: 1,
    imgSrc: process.env.PUBLIC_URL + `/assets/gallery-content-busan-1.jpg`,
    description:
      "It's already spring in Busan! Let's go to the plum blossom destination",
  },
  {
    id: 2,
    imgSrc: process.env.PUBLIC_URL + `/assets/gallery-content-busan-2.jpg`,
    description: "Spring in Daishin Park with Cherry Blossoms",
  },
  {
    id: 3,
    imgSrc: process.env.PUBLIC_URL + `/assets/gallery-content-busan-3.jpg`,
    description: "Spring Sunrise at Hwangnyeongsan Mountain in Busan",
  },
  {
    id: 4,
    imgSrc: process.env.PUBLIC_URL + `/assets/gallery-content-busan-4.jpg`,
    description: "a spring flower trip to Busan",
  },
  {
    id: 5,
    imgSrc: process.env.PUBLIC_URL + `/assets/gallery-content-busan-5.jpg`,
    description: "Spring in Busan",
  },
  {
    id: 6,
    imgSrc: process.env.PUBLIC_URL + `/assets/gallery-content-busan-6.jpg`,
    description: "Let's go to Busan in spring",
  },
];
